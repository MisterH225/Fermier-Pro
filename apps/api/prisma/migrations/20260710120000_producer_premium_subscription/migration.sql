-- Abonnement Premium producteur + nettoyage équipes (application immédiate Free)
...
CREATE INDEX IF NOT EXISTS "ProducerProfile_subscriptionTier_idx"
  ON "ProducerProfile"("subscriptionTier");
CREATE INDEX IF NOT EXISTS "ProducerProfile_subscriptionTier_nextBillingAt_idx"
  ON "ProducerProfile"("subscriptionTier", "nextBillingAt");
CREATE INDEX IF NOT EXISTS "ProducerProfile_subscriptionStatus_idx"
  ON "ProducerProfile"("subscriptionStatus");
...
-- Tous les producteurs passent Free (choix explicite requis pour Premium)
UPDATE "ProducerProfile"
SET "subscriptionTier" = 'free',
    "subscriptionStatus" = NULL,
    "subscriptionChosenAt" = COALESCE("subscriptionChosenAt", NOW());

-- Retrait immédiat des collaborateurs (hors propriétaire)
DELETE FROM "FarmMembership"
WHERE "role" <> 'owner';

-- Gel des invitations en cours
