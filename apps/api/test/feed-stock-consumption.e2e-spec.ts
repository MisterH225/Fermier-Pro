...
describeOrSkip("Stock aliment — consommation avec entrées (e2e)", () => {
  ...
  it("reflète une entrée postérieure au dernier contrôle dans le stock courant", async () => {
    ...
    // Entrée postérieure au contrôle → stock = niveau entrée (900 kg), pas figé au contrôle
  ...
  it("affiche le stock de l'entrée quand elle est plus récente qu'un contrôle à 0", async () => {
    const typeRes = await request(app.getHttpServer())
      .post(`/api/v1/farms/${ctx.farmId}/feed/types`)
      .set("Authorization", `Bearer ${ctx.token}`)
      .send({
        name: "Aliment entrée après contrôle e2e",
        unit: "sac",
        weightPerBagKg: 25,
        color: "#334455"
      });
    const refillTypeId = typeRes.body.id as string;
    ...
    const stats = await request(app.getHttpServer())
      ...
      .set("Authorization", `Bearer ${ctx.token}`);
    const row = (stats.body.items as Array<Record<string, unknown>>).find(
      (x) => x.feedTypeId === refillTypeId
    );
    expect(Number.parseFloat(String(row?.currentStockKg ?? ""))).toBeCloseTo(1000, 1);
    expect(row?.hasSufficientData).toBe(false);
    expect(row?.avgDailyConsumptionKg).toBeNull();

    await ctx.prisma.feedStockMovement.deleteMany({
      where: { feedTypeId: refillTypeId }
    });
    await ctx.prisma.feedType.delete({ where: { id: refillTypeId } });
  });
