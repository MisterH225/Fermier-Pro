...
import { resolveProducerPremiumBillingConfig } from "../producer-subscription/producer-premium-billing-config";
...
@Injectable()
export class AdminProducerSubscriptionsService {
  constructor(
  ...
  async list(params: { status?: string; q?: string; take?: number }) {
      ...
      include: {
        user: {
          select: {
            ...
            email: true,
            phone: true,
            ownedFarms: { where: { status: "active" }, select: { id: true } }
          }
        }
      }
    });
    ...
    return {
      ...
      items: rows.map((p) => ({
        ...
        email: p.user.email,
        phone: p.user.phone,
        farmCount: p.user.ownedFarms.length,
  ...
  async listInvoices(params: {
    ...
    const take = Math.min(100, Math.max(1, params.take ?? 50));
    const where: Prisma.ProducerSubscriptionInvoiceWhereInput = {};

    if (params.status?.trim()) {
      where.status = params.status.trim() as MerchantSubscriptionInvoiceStatus;
    }
    if (params.profileId?.trim()) {
      where.producerProfileId = params.profileId.trim();
    }
    if (params.q?.trim()) {
      const q = params.q.trim();
    ...
    const rows = await this.prisma.producerSubscriptionInvoice.findMany({
      where,
      take,
  ...
  async getInvoice(invoiceId: string, verify = false) {
    const row = await this.prisma.producerSubscriptionInvoice.findUnique({
      where: { id: invoiceId },
      include: {
        producerProfile: {
          include: {
    ...
    if (!row) {
      throw new NotFoundException("Facture abonnement producteur introuvable");
    }
  ...
  private async inspectProviderPayment(
    invoice: Prisma.ProducerSubscriptionInvoiceGetPayload<{
      include: {
        producerProfile: {
          include: {
            user: {
    ...
    if (
      providerRef.startsWith("producer-premium:") ||
      providerRef.startsWith("producer-sub-wallet:")
    ) {
      return {
        checkedAt,
        providerRef,
  ...
  private serializeInvoice(
    row: Prisma.ProducerSubscriptionInvoiceGetPayload<{
      include: {
        producerProfile: {
          include: {
            user: {
    ...
    const profile = row.producerProfile;
  ...
  private async getOne(profileId: string) {
    const p = await this.prisma.producerProfile.findUnique({
      where: { id: profileId },
      include: {
        user: {
          select: {
