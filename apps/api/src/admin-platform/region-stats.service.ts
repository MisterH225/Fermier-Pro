...
export class RegionStatsService {
  ...
  async getRegionalMortality(query: RegionalStatsQueryDto) {
  ...
  async getRegionalHerd(query: RegionalStatsQueryDto) {
    const { from, to, rows, coverage } = await this.loadAggregated(query);
    const departments = suppressLowCells(
      rows.map((row) => ({
        departmentCode: row.departmentCode,
  ...
  async getRegionalReproduction(query: RegionalStatsQueryDto) {
    const { from, to, rows, coverage } = await this.loadAggregated(query);
    const departments = suppressLowCells(
      rows.map((row) => ({
        departmentCode: row.departmentCode,
  ...
  async getRegionalGrowth(query: RegionalStatsQueryDto) {
    const { from, to, rows, coverage } = await this.loadAggregated(query);
    const departments = suppressLowCells(
      rows.map((row) => ({
        departmentCode: row.departmentCode,
  ...
  async getRegionalVetCoverage(query: RegionalStatsQueryDto) {
    const { from, to, rows, coverage } = await this.loadAggregated(query);
    const departments = suppressLowCells(
      rows.map((row) => ({
        departmentCode: row.departmentCode,
  ...
  private aggregateSnapshots(
    ...
    const byDept = new Map<string, AggregatedDeptRow & { gmqWeight: JsonRecord }>();
  ...
  private buildCoverage(rows: AggregatedDeptRow[]): Coverage {
    let farmCount = 0;
    let animalCount = 0;
  ...
  /** z-score mortalité semaine courante vs moyenne mobile 8 semaines précédentes. */
