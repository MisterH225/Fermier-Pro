...
export function EditFarmRevenueScreen({ route, navigation }: Props) {
  ...
  const revenueQuery = useQuery({
    queryKey: ["farmRevenue", farmId, revenueId, activeProfileId],
    queryFn: () =>
      fetchFarmRevenue(accessToken, farmId, revenueId, activeProfileId),
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void qc.invalidateQueries({
        queryKey: ["farmRevenue", farmId, revenueId]
      });
      navigation.goBack();
    },
    onError: (e: Error) => {
  ...
  if (err || !revenueQuery.data) {
    return (
      <View style={styles.centered}>
        <Text style={styles.error}>{err ?? "Revenu introuvable."}</Text>
      </View>
    );
  }
