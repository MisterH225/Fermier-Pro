...
import { mobileColors, mobileRadius, mobileShadows, mobileSpacing, mobileTypography, mobileKpiPalette, mobileStatusSurfaces, mobileFontSize } from "../theme/mobileTheme";
...
export function FarmFinanceScreen({ route, navigation }: Props) {
  ...
  const goEdit = useCallback(
    (txRow: FinanceMergedTransactionDto) => {
      ...
      if (txRow.isAutoGenerated || txRow.source === "auto") {
        Alert.alert(
          t("financeScreen.autoGenerated"),
          t("financeScreen.autoGeneratedLocked")
        );
        return;
      }
  ...
  const financeEventItems = useMemo((): EventItem[] => {
    return recentForTab.map((txRow) => {
      ...
      return {
        ...
        title: txRow.label,
        subtitle: `${txRow.kind === "income" ? t("financeScreen.income") : t("financeScreen.expense")}${txRow.categoryLabel ? ` · ${txRow.categoryLabel}` : ""}${txRow.isAutoGenerated ? ` · ${t("financeScreen.autoGenerated")}` : ""}`,
        value: signed,
    ...
    (item: EventItem, { close }: { close: () => void }) => {
      ...
      return (
        <View style={{ paddingBottom: mobileSpacing.md }}>
          ...
          {txRow.isAutoGenerated || txRow.source === "auto" ? (
            <View
              style={{
                ...
                backgroundColor: producerColors.financeIndigoBg
              }}
            >
              <Text style={{ fontSize: mobileFontSize.sm, fontWeight: "700", color: producerColors.financeIndigoText }}>
                {t("financeScreen.autoGenerated")}
              </Text>
            </View>
          ) : (
...
const styles = StyleSheet.create({
  ...
  legendDot: { width: 8, height: 8, borderRadius: mobileRadius.sm },
  ...
  rowAmount: { fontSize: mobileFontSize.lg, fontWeight: "800", color: mobileColors.textPrimary },
  rowMeta: {
  ...
  lineItem: { fontSize: mobileFontSize.md, color: mobileColors.textPrimary, marginBottom: 4 },
  ...
  stackLab: { fontSize: mobileFontSize.sm, marginBottom: 4, color: mobileColors.textPrimary },
  stackBar: {
    flexDirection: "row",
    height: 10,
    borderRadius: mobileRadius.sm,
