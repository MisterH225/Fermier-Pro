...
import { CreateGestationModal } from "../shared/CreateGestationModal";
import { GestationDetailModal } from "./GestationDetailModal";
import { MiseBasModal } from "./MiseBasModal";
import {
  CheptelStyleKpiCard,
  cheptelKpiGridStyles
} from "../cheptel/overview/CheptelStyleKpiCard";
import { SailliePlanningAI } from "./SailliePlanningAI";
...
export type FarmGestationPanelProps = {
  farmId: string;
  farmName: string;
  ...
  embedded?: boolean;
  /** Called when the create-gestation affordance should be available (standalone header). */
  onCreateRequestChange?: (openCreate: (() => void) | null, readOnly: boolean) => void;
};

type TabId = FarmGestationPanelTabId;
...
export function FarmGestationPanel({
  farmId,
  farmName,
  navigation,
  initialTab,
  ...
  highlightSowId,
  embedded = false,
  onCreateRequestChange
}: FarmGestationPanelProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setHighlightSow(null), 2200);
  ...
  useEffect(() => {
    if (!onCreateRequestChange) {
      return;
    }
    if (readOnly) {
      onCreateRequestChange(null, true);
      return;
    }
    onCreateRequestChange(() => setCreateOpen(true), false);
    return () => onCreateRequestChange(null, readOnly);
  }, [onCreateRequestChange, readOnly]);

  // silence unused when embedded (prop reserved for callers / future chrome)
  void embedded;

  const enabled = Boolean(accessToken) && !(techPerms.isTech && !techPerms.canView);

  const overviewQ = useQuery({
    queryKey: ["gestation", farmId, "overview", activeProfileId],
    queryFn: () =>
      fetchGestationOverview(accessToken!, farmId, activeProfileId),
    enabled: enabled && !(techPerms.isTech && techPerms.loading)
  });
